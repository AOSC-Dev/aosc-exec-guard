...
systemd-install-test:
    ...
    step '安装 conf（走 just install 生成过滤后的 conf，模拟打包安装）'
    # install 配方会按本机家族删掉本机架构的规则（不删的话条目会劫持解释器自身）。
    # F 标志要求解释器文件在注册时就存在（这里指向本仓库构建的二进制）。
    PKG=$(mktemp -d)
    just install "$PKG" > /dev/null
...
install prefix="" target_arch="" ignore_family="no":
    ...
    # 目标架构默认 uname -m（交叉打包时显式给）；ignore-family=yes 对应 qemu 的同名选项：
    # 同家族但本机跑不了的目标（如 riscv64 上的 riscv32）。
    set -euo pipefail
    ...
    # uname -m / 参数里的写法 → 我们的条目名（同 qemu 的 qemu_normalize）。
    normalize_arch() {
      case "$1" in
        i[3-6]86) echo i386 ;;
        amd64) echo x86_64 ;;
    ...
    if [ -z "$HOST_FAMILY" ]; then
      ...
      echo "请把目标架构换成 <x86_64|aarch64|riscv64|mips64|…>。" >&2
      exit 1
    fi
    ...
    [ -x "$BIN_SRC" ] || {
      echo "找不到 $BIN_SRC；先跑 just build。" >&2
      exit 1
    }
    ...
    if [ "$HOST_ARCH" = "$LOCAL_ARCH" ]; then
      ...
      echo "注意：目标架构 $HOST_ARCH 与本机（$LOCAL_ARCH）不同，跳过\"规则命中解释器自身\"的本地检查。" >&2
    fi
    ...
    if [ "$staging" = yes ]; then
      echo '（staging 模式：没有碰内核；装到目标系统时在那里执行 just install）'
      exit 0
    fi
...
uninstall prefix="":
    #!/usr/bin/env bash
    # 前缀为空或 /usr 时会重启 systemd-binfmt 并注销剩下的条目。
    set -euo pipefail
